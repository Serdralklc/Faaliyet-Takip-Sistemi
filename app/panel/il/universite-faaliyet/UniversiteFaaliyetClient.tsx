...
import { UNI_KATEGORILER, UNI_KATEGORI_LABEL, UNI_KATEGORI_RENK, type UniKategoriKey } from "@/lib/universite-faaliyet";
...
type FormState = {
  tarih: string; donem: string; kategori: UniKategoriKey; adSecim: string; manuelAd: string;
...
const bosForm = (donem = "DONEM_1"): FormState => ({
  tarih: bugunStr(), donem, kategori: "ILIM_SOHBET", adSecim: UNI_KATEGORILER[0].adlar[0] ?? "", manuelAd: "",
  aciklama: "", yer: "", katilimci: "", ilkKezKatilan: "", yeniIntisap: "",
  fotoFile: null, dosyaFile: null, fotoSil: false, dosyaSil: false,
});

export function UniversiteFaaliyetClient({ ilId, ilAd, bolgeAd }: { ilId: string; ilAd: string; bolgeAd: string }) {
  ...
  function load() {
    setLoading(true);
    fetch(`/api/universite-faaliyetler?ilId=${ilId}&yil=${yil}&donem=${donem}`)
      .then(r => (r.ok ? r.json() : []))
  ...
  const kategoriDef = UNI_KATEGORILER.find(k => k.key === form.kategori)!;
  ...
  function openEdit(f: Faaliyet) {
    const def = UNI_KATEGORILER.find(k => k.key === f.kategori)!;
  ...
  function setKategori(k: UniKategoriKey) {
    const def = UNI_KATEGORILER.find(x => x.key === k)!;
  ...
  async function submit() {
    ...
    const res = await fetch(editing ? `/api/universite-faaliyetler/${editing.id}` : "/api/universite-faaliyetler", {
  ...
  async function sil() {
    if (!silId) return;
    setSiliniyor(true);
    const res = await fetch(`/api/universite-faaliyetler/${silId}`, { method: "DELETE" });
    setSiliniyor(false);
    setSilId(null);
  ...
  const columns: DataTableColumn<Faaliyet>[] = [
    { key: "tarih", header: "Tarih", mobile: true, sortValue: f => f.tarih, render: f => <span className="font-semibold text-heading whitespace-nowrap">{trTarih(f.tarih)}</span> },
    {
      ...
      render: f => <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold" style={{ background: `${UNI_KATEGORI_RENK[f.kategori]}1a`, color: UNI_KATEGORI_RENK[f.kategori] }}>{UNI_KATEGORI_LABEL[f.kategori]}</span>,
    },
    ...
    { key: "yeniIntisap", header: "Yeni İntisap", sortValue: f => f.yeniIntisap, render: f => <span className="font-bold" style={{ color: "var(--accent)" }}>{f.yeniIntisap}</span> },
    {
      ...
      render: f => (
        <div className="flex items-center gap-2">
          {f.fotoKey && <a href={`/api/universite-dosya/${f.id}?tip=foto`} target="_blank" rel="noreferrer" title="Foto" onClick={e => e.stopPropagation()}><ImageIcon size={15} className="text-blue-500" /></a>}
          {f.dosyaKey && <a href={`/api/universite-dosya/${f.id}?tip=dosya`} target="_blank" rel="noreferrer" title={f.dosyaAd ?? "Dosya"} onClick={e => e.stopPropagation()}><Paperclip size={15} className="text-amber-600" /></a>}
    ...
    <div className="p-6 lg:p-8 space-y-5 max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="sv-page-header" style={{ marginBottom: 0 }}>
          <h1>Üniversite Gençlik Faaliyetleri</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Toplam Katılımcı", val: ozet.toplamKatilimci, renk: "#1D4ED8" },
      ...
      {ozet.toplam > 0 && (
        <div className="flex flex-wrap gap-2">
          {UNI_KATEGORILER.filter(k => ozet.perKat[k.key]?.sayi).map(k => (
      ...
      {loading ? (
        ...
        <DataTable
          ...
          columns={columns}
          rowKey={f => f.id}
          searchText={f => `${f.faaliyetAdi} ${UNI_KATEGORI_LABEL[f.kategori]} ${f.yer ?? ""}`}
          searchPlaceholder="Faaliyet, kategori veya yer ara…"
          emptyText="Bu dönem için henüz faaliyet kaydı yok. “Faaliyet Ekle” ile başlayın."
          onRowClick={openEdit}
          rowActions={f => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} maxWidth={620}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Select label="Kategori" required value={form.kategori} onChange={e => setKategori(e.target.value as UniKategoriKey)}>
              {UNI_KATEGORILER.map(k => <option key={k.key} value={k.key}>{k.label}</option>)}
            </Select>
          </div>
