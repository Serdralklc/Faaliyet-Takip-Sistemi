...
export function LiseRaporClient({ bolgeler, faaliyetler, yillar }: {
  ...
  const bolgeOzet = useMemo(() => {
    ...
    return gosterilenBolgeler.map(b => ({ bolge: b, ozet: liseOzet(grup.get(b.id) ?? []) }));
        ...
        <table className="w-max min-w-full text-sm">
          ...
          <tbody>
            {bolgeOzet.map(({ bolge, ozet }) => (
              <tr key={bolge.id} className="border-t" style={{ borderColor: "var(--border)" }}>
            ...
            {/* Türkiye toplam — yalnızca tüm bölgeler görünürken */}
            {!bolgeId && (
              <tr style={{ background: "rgba(11,107,58,0.08)" }}>
                <td className="px-3 py-3 font-black whitespace-nowrap sticky left-0 z-10" style={{ color: "#0B6B3A", background: "#DCEEE2" }}>🇹🇷 Türkiye</td>
            ...
            {turkiye.toplam === 0 && (
              <tr><td colSpan={LISE_KATEGORILER.length + 5} className="px-4 py-8 text-center text-sm" style={{ color: "var(--text-muted)" }}>Seçili filtre için henüz lise gençlik faaliyeti kaydı yok.</td></tr>
            )}
          </tbody>
        </table>
        </div>
