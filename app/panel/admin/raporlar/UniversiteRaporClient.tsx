...
export function UniversiteRaporClient({ bolgeler, faaliyetler, yillar }: {
  ...
  const bolgeOzet = useMemo(() => {
    ...
    return bolgeler.map(b => ({ bolge: b, ozet: uniOzet(grup.get(b.id) ?? []) }));
    ...
    <div className="p-6 space-y-5 max-w-[1400px]">
      ...
      <div className="sv-section overflow-x-auto">
        ...
        <table className="w-full text-sm">
          <thead style={{ background: "var(--bg-th)" }}>
            <tr>
              {["Bölge", ...UNI_KATEGORILER.map(k => k.label), "Toplam", "Katılımcı", "İlk Kez", "Y. İntisap"].map(h => (
          ...
          <tbody>
            ...
            <tr style={{ background: "rgba(29,78,216,0.08)" }}>
              <td className="px-3 py-3 font-black whitespace-nowrap" style={{ color: "#1D4ED8" }}>🇹🇷 Türkiye</td>
            ...
            {turkiye.toplam === 0 && (
              <tr><td colSpan={UNI_KATEGORILER.length + 5} className="px-4 py-8 text-center text-sm" style={{ color: "var(--text-muted)" }}>{yil} yılı için henüz üniversite gençlik faaliyeti kaydı yok.</td></tr>
            )}
          </tbody>
        </table>
      </div>
