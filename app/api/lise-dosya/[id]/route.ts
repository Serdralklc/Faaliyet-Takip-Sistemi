...
/** Lise faaliyet foto/dosya eki — il sorumlusu (kendi ili), bölge (kendi illeri), yönetici */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const f = await prisma.liseFaaliyet.findUnique({
    where: { id },
    select: {
      ilId: true,
      fotoKey: true, fotoUrl: true, fotoMime: true,
      dosyaKey: true, dosyaUrl: true, dosyaMime: true, dosyaAd: true, dosyaBoyut: true,
  ...
  // Blob: mutlak URL'e yönlendir; yerel: stream
