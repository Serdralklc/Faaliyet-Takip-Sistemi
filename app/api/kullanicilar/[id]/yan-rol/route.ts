...
import { rolAtayabilir, icerikYoneticisiAtayabilir, YAN_ROLLER, YAN_ROL_LABEL } from "@/lib/constants";
...
// POST: bir yönetim kullanıcısına yan rol ver/al (çoklu).
// Rol atama yetkisi olanlar (Sistem Admini, Türkiye Eğitim Sorumlusu, İçerik Yöneticisi)
// yan rolleri değiştirebilir; ANCAK "İçerik Yöneticisi" yan rolünü verme/alma yalnızca
// Sistem Admini'ne özeldir (yetki yükseltmeyi önler).
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!rolAtayabilir(session.user.role, session.user.icerikYoneticisi)) {
    return NextResponse.json({ error: "Yan rol değiştirme yetkiniz yok" }, { status: 403 });
  }
  ...
  // İçerik Yöneticisi yan rolünü verme/alma yalnızca Sistem Admini'ne özel
  if (yanRol === "ICERIK_YONETICISI" && !icerikYoneticisiAtayabilir(session.user.role)) {
    return NextResponse.json({ error: "İçerik Yöneticisi yan rolünü yalnızca Sistem Admini değiştirebilir" }, { status: 403 });
  }
