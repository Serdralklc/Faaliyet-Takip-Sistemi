...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const mevcut = await prisma.universiteFaaliyet.findUnique({ where: { id } });
  ...
  const data: Record<string, unknown> = {
    tarih, yil: tarih.getFullYear(), donem: gecerliDonem(form.get("donem")), kategori, faaliyetAdi,
  ...
  const f = await prisma.universiteFaaliyet.update({ where: { id }, data });
  ...
  await createAuditLog({
    userId: session.user.id, action: ACTIONS.UNI_FAALIYET_UPDATED,
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.universiteFaaliyet.delete({ where: { id } });
  if (mevcut.fotoKey) await deleteFile(mevcut.fotoKey);
  if (mevcut.dosyaKey) await deleteFile(mevcut.dosyaKey);

  await createAuditLog({
    userId: session.user.id, action: ACTIONS.UNI_FAALIYET_DELETED,
    entity: "UniversiteFaaliyet", entityId: id,
    ipAddress: req.headers.get("x-forwarded-for") ?? undefined,
  });
